Dado('Acesso a tela de relogios para atualizar a configuração de pt') do
  ...
  Quando('atualizo a configuração de pt') do
  ...
  Quando('Aperto no botão salvar para atualizar a configuração de pt') do
  ...
  Então('Aguardo a confirmação de atualizacao da configuração de pt {string}') do |message_sucess|
