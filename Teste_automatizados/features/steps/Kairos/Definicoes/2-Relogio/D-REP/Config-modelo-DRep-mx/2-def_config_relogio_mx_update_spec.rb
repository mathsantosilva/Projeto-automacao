Dado('Acesso a tela de relogios para atualizar a configuração de mx') do
...
Quando('atualizo a configuração de mx') do
...
Quando('Aperto no botão salvar para atualizar a configuração de mx') do
...
Então('Aguardo a confirmação de atualizacao da configuração de mx {string}') do |message_sucess|
