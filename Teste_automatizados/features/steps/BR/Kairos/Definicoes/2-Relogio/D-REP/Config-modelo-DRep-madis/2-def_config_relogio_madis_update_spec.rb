Dado('Acesso a tela de relogios para atualizar a configuração da madis') do
  ...
  Quando('atualizo a configuração da madis') do
  
    #Selecionando o botão alterar
    @common.select_button('body > div.ZonaConteudo > div.Conteudo > table > tbody > tr:nth-child(2)','img[class="pointer icons editIcon"]')
  ...
  Quando('Aperto no botão salvar para atualizar a configuração da madis') do
  ...
  Então('Aguardo a confirmação de atualizacao da configuração da madis {string}') do |message_sucess|
