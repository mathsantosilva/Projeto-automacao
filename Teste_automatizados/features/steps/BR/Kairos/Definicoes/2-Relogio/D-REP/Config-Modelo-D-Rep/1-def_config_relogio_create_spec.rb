Dado('Acesso a tela de relogios para criar a configuração') do
  ...
  Quando('Cadastrando uma nova configuração') do
  ...
  Quando('Aperto no botão salvar a configuração') do
  ...
  Então('Aguardo a confirmação de criacao da configuração {string}') do |message_sucess|
